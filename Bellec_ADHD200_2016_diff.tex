...
%DIF DEL ../../gdrive/articles/Bellec_ADHD200/Bellec_ADHD200_2016-submitted/Bellec_ADHD200_2016.tex   Fri Mar  4 11:11:35 2016
%DIF ADD Bellec_ADHD200_2016.tex                                                                      Fri May 27 22:41:06 2016
...
\usepackage{rotating}
%DIF 33c33
%DIF < \usepackage[colorinlistoftodos]{todonotes}
%DIF -------
...
\DIFaddbegin \author[label0,label1,label5]{\DIFadd{Yassine Benhajali}\DIFaddend }
\DIFaddbegin \address[label5]{D\'epartement d'Anthropologie, Universit\'e de Montr\'eal, Montr\'eal, CA}
...
Shared preprocessed data and extracted features include:
 \begin{itemize} 
\tightlist
    \item 3D grey matter density maps suitable for voxel-based morphometry -- Athena and Burner (see Figure~\ref{fig:burner}\DIFaddbegin \DIFadd{)}\DIFaddend ,
...
\begin{center}
  ...
  \caption{\DIFdelbeginFL \DIFdelFL{Grey }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Average and standard deviation of the grey }\DIFaddendFL matter density maps generated by the Burner pipeline\DIFdelbeginFL \DIFdelFL{. An example of an individual map is presented}\DIFdelendFL , \DIFdelbeginFL \DIFdelFL{along with the average and standard deviation maps }\DIFdelendFL for all subjects in the test subsample of ADHD-200 Preprocessed. }
  \label{fig:burner}
\end{center}
\end{figure}
...
\DIFaddbegin \DIFadd{Images were visually inspected and attempts were made to fix gross misregistration errors by hand adjusting the offending images to center them on the anterior commisure and rotate them into rough correspondence with the MNI template. }\DIFaddend With the exception of a few datasets that were missing data, and one dataset that was corrupted, all of the ADHD-200 Sample was processed and released by Athena, regardless of data quality. This was done to \DIFdelbegin \DIFdel{accomodate }\DIFdelend \DIFaddbegin \DIFadd{accommodate }\DIFaddend differing opinions as to what qualifies as usable data, and to provide poor quality data that may be used by others to develop methods that are robust to noise. Files containing the six \DIFdelbegin \DIFdel{mostion }\DIFdelend \DIFaddbegin \DIFadd{motion }\DIFaddend parameters for each rs-fMRI data and anatomical and mean EPI images in template space have been included in the release to enable users to determine high-motion data or poor registrations.
\DIFaddbegin \par
\DIFaddend Additional quality metrics derived from the \DIFaddbegin \DIFadd{preprocessed }\DIFaddend s-MRI, GM masks, mean rs-fMRI images, fALFF maps\DIFaddbegin \DIFadd{, }\DIFaddend and FC maps were also included to help with the QC process. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend For each data type a mean and standard deviation image was calculated from all of the scans \DIFaddbegin \DIFadd{(in stereotaxic space) }\DIFaddend from all of the subjects. These images were used to perform a voxelwise z-score transformation on each data type for each subject $z_i = \frac{(v_i-m_i)}{\sigma_i}$. \DIFdelbegin \DIFdel{The absolute values }\DIFdelend \DIFaddbegin \DIFadd{For each image (or map), the number }\DIFaddend of these z-scores \DIFdelbegin \DIFdel{were thresholded at }\DIFdelend \DIFaddbegin \DIFadd{whose absolute value exceeded }\DIFaddend 3 \DIFdelbegin \DIFdel{, and summed across voxels for }\DIFdelend \DIFaddbegin \DIFadd{where summed to generate }\DIFaddend a quality score\DIFdelbegin \DIFdel{for that image}\DIFdelend . The higher the resulting sum, the larger the number of voxels a image has \DIFdelbegin \DIFdel{that are $\geq 3$ }\DIFdelend \DIFaddbegin \DIFadd{with $\left|z\right| \geq 3$ }\DIFaddend and the more likely the images are outliers. Using this metric it is possible to rank images \DIFdelbegin \DIFdel{, and allows }\DIFdelend for a more directed search for poor quality scans. These metrics are distributed in participant-specific text files along with the BASH scripts that implement the procedure.
\DIFaddbegin \par
\DIFadd{A rigorous visual inspection of registration has also been performed using the same procedures and raters (PB and YB) as the NIAK pipeline. The procedure entailed generating an online report}\footnote{\url{http://preprocessed-connectomes-project.org/adhd200_visual_qc_athena/}} \DIFadd{of registration quality for all subjects.  The report includes a visualization of the individual structural images in standard space overlaid onto the corresponding brain template (MNI pediatric), as well as a visualization of the individual mean EPI image overlaid onto the corresponding structural image, both in standard space. These images were visually reviewed, and images with severe misregistraion or quality problems were marked as ``fail'' ($7\%$ failure rate). The outcome of the visual quality assessment along with all available phenotypic variables have been consolidated into a tabular-separated (.tsv) spreadsheet that can be downloaded from NITRC}\footnote{\url{https://www.nitrc.org/frs/download.php/9024/adhd200_preprocessed_phenotypics.tsv}}\DIFadd{.
}\DIFaddend 
...
Outputs of the NIAK pipeline were subjected to a careful visual inspection and the \DIFdelbegin \DIFdel{results quality calls}\DIFdelend \DIFaddbegin \DIFadd{QC reports}\DIFaddend , along with head motion statistics, are available on the NIAK description page \footnote{\url{http://www.nitrc.org/plugins/mwiki/index.php/neurobureau:NIAKPipeline#Quality_control_of_the_preprocessing_-_Training_dataset}}. Estimates of the maximum motion (translation and rotation) between consecutive functional volumes for each rs-fMRI dataset were inspected to categorize the datasets as containing minimal (\DIFdelbegin \DIFdel{<}\DIFdelend \DIFaddbegin \DIFadd{$<$}\DIFaddend 1mm or degree), moderate (2 to 3 mm or degrees) or severe motion (\DIFdelbegin \DIFdel{>}\DIFdelend \DIFaddbegin \DIFadd{$>$}\DIFaddend 3 mm or degrees). The individual \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{brain registration }\DIFaddend of the NIAK pipeline were visually inspected \DIFdelbegin \DIFdel{by M. Chouinard-Decortes for quality of the registration between rs-fMRI and s-MRI data, registration of s-MRI data to template space, and for common artefacts such as ghosting and signal loss}\DIFdelend \DIFaddbegin \DIFadd{using online QC reports}\footnote{\url{http://preprocessed-connectomes-project.org/adhd200_visual_qc_niak/}}\DIFadd{, similar to those generated for the Athena pipeline}\DIFaddend . When substandard registration outcomes were identified, a parameter controlling the non-uniformity correction of the s-MRI was adjusted and the analysis was repeated until the coregistration results were satisfactory. \DIFdelbegin \DIFdel{To aid in further quality control, the motion statistics are distributed in comma-seperated values format (.csv) for each site. Additionally the average structural and functional scans after linear and non-linear transformations and the average of all anatomical and functional brain masks for each site of the training and test samples are included as compressed nifti format (.nii.gz) \mbox{%DIFAUXCMD
\cite{lavoie2012integration}}%DIFAUXCMD
. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Satisfactory results could not be achived with some datasets and have been indicated as ``Fail'' for QC ($5.2\%$ failure rate) in the .tsv spreadsheet including QC assessments, available on NITRC}\footnote{\url{https://www.nitrc.org/frs/download.php/9024/adhd200_preprocessed_phenotypics.tsv}}\DIFadd{. 
}\DIFaddend %%%
%%% - tables
%%%
...
\begin{center}
  ...
  \caption{\DIFaddFL{Statistics on download and citations of the ADHD200 preprocessed initiative.}}
  \label{fig:usage}
\end{center}
\end{figure}
\DIFaddend 

The ADHD-200 Preprocessed initiative was successful in \DIFaddbegin \DIFadd{terms of }\DIFaddend its primary objectives: the derivatives shared in the repository were effectively used by many researchers during and after the ADHD-200 Global Competition, with over 10,500 downloads by more than 600 users, as well as \DIFdelbegin \DIFdel{49 resulting publications \mbox{%DIFAUXCMD
\cite{Rangarajan2014, Liang2012, Tabas2014, Rangarajan2015, Mahanand2013, Lifshitz2012, Fujita2013, Ji2011, Li2015, Li2013, Liu2012, DosSantosSiqueira2014, Olivetti2014, Han2015, Wang2013a, Subramanian2013, Dey2014, Bellec2012, Bohland2012, Chang2012, Cheng2012, Colby2012, Dai2012, Dey2012, Eloyan2012, Olivetti2012, Sato2012a, Carmona2015, Carmona2015a, Hou2015, Deshpande2015, She2014, Lavoie-Courchesne2012b, Chen2015, Nunez-Garcia2015, Solmaz2012, Anderson2014, KadkhodaeianBakhtiari2012, Sato2013, Kyeong2015, Sato2012, Takahashi2012, He2013, Kong2013, Yao2013, Yang2015, Ahn2015, Fujita2014, Reiss2014}}%DIFAUXCMD
, three }\DIFdelend \DIFaddbegin \DIFadd{52 resulting publications \mbox{%DIFAUXCMD
\cite{Rangarajan2014, Liang2012, Tabas2014, Rangarajan2015, Mahanand2013, Lifshitz2012, Fujita2013, Ji2011, Li2015, Li2013, Liu2012, DosSantosSiqueira2014, Olivetti2014, Han2015, Wang2013a, Dey2014, Bellec2012, Bohland2012, Chang2012, Cheng2012, Colby2012, Dai2012, Dey2012, Eloyan2012, Olivetti2012, Sato2012a, Subramanian2013, Carmona2015, Hou2015, Deshpande2015, She2014, Lavoie-Courchesne2012b, Chen2015, Nunez-Garcia2015, Solmaz2012, Anderson2014, KadkhodaeianBakhtiari2012, Sato2013, Kyeong2015, Sato2012, Takahashi2012, He2013, Kong2013, Yao2013, Yang2015, Ahn2015, Fujita2014, Reiss2014,nachamai2016sub,Brown20161238,Yu2016,Fujita2015}}%DIFAUXCMD
, 3 }\DIFaddend PhD theses \cite{Colby2012a, Dey2014phd, Zhang2012}, \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{3 }\DIFaddend master's dissertations \cite{VanGalenLast2011, Vidal2014, Wang2013}\DIFdelbegin \DIFdel{and one }\DIFdelend \DIFaddbegin \DIFadd{, and 1 }\DIFaddend patent \cite{Dey2013} derived from the release in just over three years\DIFdelbegin \DIFdel{, with further publications }\DIFdelend \DIFaddbegin \DIFadd{. Further publications are either }\DIFaddend in press or under review\DIFaddbegin \DIFadd{, and an updated list of publications will be maintained in a public Mendeley group}\footnote{\url{https://www.mendeley.com/groups/4198361/adhd-200-preprocessed/}}\DIFadd{. Although there was clearly a peak in usage around the ADHD-200 Global competition, there has been a sustained amount of downloads and publications since then, see Figure \ref{fig:usage}, which we take as a demonstration of a long-term interest from the community in this resource}\DIFaddend . 
\DIFdelbegin \DIFdel{The resourcealso expanded }\DIFdelend \DIFaddbegin \par
\DIFadd{The ADHD-200 preprocessed has helped to expand }\DIFaddend the boundaries of the traditional neuroimaging community, with several publications in \DIFaddbegin \DIFadd{core }\DIFaddend engineering and statistics journals that do not routinely feature neuroimaging applications\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.][]{Liang2012, Rangarajan2015, Rangarajan2014, Mahanand2013, Ji2011, Li2015, Li2013, Liu2012, Subramanian2013, Hou2015, Deshpande2015, She2014, Chen2015, He2013, Kong2013, Yang2015, Ahn2015}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{, such as Statistica Sinica \mbox{%DIFAUXCMD
\citep{Yang2015} }%DIFAUXCMD
or the Journal of the American Statistical Association \mbox{%DIFAUXCMD
\citep{Chen2015}}%DIFAUXCMD
, amongst others \mbox{%DIFAUXCMD
\citep[e.g.][]{Liang2012, Rangarajan2015, Rangarajan2014, Mahanand2013, Ji2011, Li2015, Li2013, Liu2012, Subramanian2013, Hou2015, Deshpande2015, She2014, He2013, Kong2013, Ahn2015}}%DIFAUXCMD
}\DIFaddend . In particular, the winning team of the ADHD-200 Global Competition was based at the Johns Hopkins Biostatistics Department and used ADHD-200 Preprocessed to develop their diagnostic algorithm \cite{Eloyan2012}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We also found interesting that a handful of methodological publications used the ADHD-200 Preprocessed sample as one application in a series of benchmarks that could be as varied as positron emission tomography in Alzheimer's disease \mbox{%DIFAUXCMD
\citep{Yang2015}}%DIFAUXCMD
, the Yeast gene regulatory network \mbox{%DIFAUXCMD
\citep{She2014} }%DIFAUXCMD
or gene expression in brain tissues of patients with HIV-1 associated neurocognitive disorders \mbox{%DIFAUXCMD
\citep{Liu2012}}%DIFAUXCMD
. This suggests that fully processed, easy-to-access imaging samples could help validate general-purpose methods on a wider scope of applications.
}\par
\DIFaddend The impact of the ADHD-200 Preprocessed repository demonstrated the need for reducing computational barriers to participation in discovery neuroscience, including but not limited to machine learning competitions based on neuroimaging data. \DIFdelbegin \DIFdel{We do not plan to expand or update }\DIFdelend \DIFaddbegin \DIFadd{While }\DIFaddend the ADHD-200 Preprocessed \DIFdelbegin \DIFdel{release as such, which we hope will continue to serve as a legacy benchmark dataset. We still }\DIFdelend \DIFaddbegin \DIFadd{initiative will have a long-term impact on that need, we }\DIFaddend believe that a much larger-scale effort will be necessary to unlock the full potential of openly shared neuroimaging data in the service of accelerating \DIFdelbegin \DIFdel{neuroimaging research. Current initiatives include a larger number of alternative analytical workflows as well as a careful harmonization of processing, quality control, and data packaging strategies}\DIFdelend \DIFaddbegin \DIFadd{neuroscience research. In line with the grassroots, open science ethos of the Neurobureau, new contributors interested in sharing derivatives of ADHD-200 or other open imaging data repository can contact us via our web-based forum}\footnote{\url{http://www.nitrc.org/forum/?group_id=383}}\DIFadd{. We are  actively seeking new contributions to the Preprocessed Connectomes Project, notably during Brainhack events\mbox{%DIFAUXCMD
\citep{Cameron_Craddock2016-du}}%DIFAUXCMD
. An important current project is the ABIDE preprocessed initiative}\footnote{\url{http://preprocessed-connectomes-project.org/abide/}} \DIFadd{\mbox{%DIFAUXCMD
\citep{dimartino2014}}%DIFAUXCMD
. This new resource, still under development, will include 16 different processing strategies for rs-fMRI processing, implemented across 4 different software packages, and 2 different pipelines for structural MRI processing. The release will feature an harmonized organization of processed derivatives across packages and extensive quality control. We are planning to continue to expand this line on work on other data sources in the near future}\DIFaddend . Our hope is that ADHD-200 Preprocessed and future related efforts will critically help fMRI researchers to identify optimal analytical paths for a given task. 

\paragraph{Acknowldegements} The authors would like to thank the ADHD-200 Consortium for assembling and sharing the ADHD-200 Sample and for hosting the ADHD-200 Global competition. \DIFdelbegin \DIFdel{The Athena }\DIFdelend \DIFaddbegin \DIFadd{RCC }\DIFaddend would like to thank the developers of AFNI and FSL, and the Advanced Research Computing at Virginia Tech for computational support. The scripts used to calculate ReHo were generously provided by Dr. Xinian Zuo. \DIFdelbegin \DIFdel{The Burner }\DIFdelend \DIFaddbegin \DIFadd{CC }\DIFaddend would like to thank the developers of SPM. \DIFdelbegin \DIFdel{The NIAK }\DIFdelend \DIFaddbegin \DIFadd{PB }\DIFaddend would like to thank the developers of MINC tools, the CIVET pipeline, \DIFdelbegin \DIFdel{and }\DIFdelend the CBRAIN computational infrastructure\DIFaddbegin \DIFadd{, and the computational resources provided by Compute Canada}\footnote{\url{https://computecanada.org/}} \DIFadd{and CLUMEQ}\footnote{\url{http://www.clumeq.mcgill.ca/}}\DIFadd{, which is funded in part by NSERC (MRS), FQRNT, and McGill University}\DIFaddend .
